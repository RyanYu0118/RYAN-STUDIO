...
- **后台编辑器**（需全站加载 `rs-loader.js`）：右下角 **[[Wiki 链接]]** 或 **Ctrl+Shift+K** 插入内链；也可直接写 MediaWiki 语法 **`[[player/rules|玩家规则]]`**，保存时自动转为 `[玩家规则](/archives/player/rules)`。Markdown 里输入 **`[[`** 会弹出路径补全（来自 `wiki-slugs.json`）。
- 未登录会跳转到登录页。
